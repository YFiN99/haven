...
export default function SwapBox({ onSwap, isPending }: { onSwap: (amt: string, path: string[]) => void, isPending: boolean }) {
  ...
  // 2. Logic Path: Handle Native HAV ke Wrapped
  const path = [
  ...
  return (
    <div className="w-full max-w-[480px] bg-[#0c0c0c] border border-zinc-800 rounded-[32px] p-8 shadow-2xl relative">
      ...
      <div className="bg-[#141414] p-6 rounded-[24px] border border-zinc-800 mb-1 focus-within:border-haven-pink/40 transition-all">
        <div className="flex justify-between text-[10px] font-black text-zinc-600 uppercase mb-3">
          <span>You Sell</span>
          <span className="italic">Bal: {balance?.formatted.slice(0, 6) || '0.00'}</span>
        </div>
        <div className="flex items-center gap-4">
          ...
          <select 
            ...
            className="bg-zinc-900 px-3 py-2 rounded-xl border border-zinc-800 text-xs font-black outline-none cursor-pointer hover:border-haven-pink"
          >
      ...
      <div className="bg-[#141414] p-6 rounded-[24px] border border-zinc-800 mt-1 mb-6 focus-within:border-haven-pink/40 transition-all">
        ...
        <div className="flex items-center gap-4">
          <input readOnly value={buyAmount === '0.0000' ? '' : parseFloat(buyAmount).toFixed(4)} placeholder="0.00"
            className="bg-transparent text-4xl font-black outline-none w-full text-zinc-600" />
      ...
      <div className="px-4 py-3 mb-6 bg-zinc-950/50 rounded-xl border border-zinc-900 flex justify-between items-center">
        ...
        <span className="text-[10px] font-black text-zinc-400 italic">
          {sellAmount ? `1 ${sellToken.symbol} = ${(parseFloat(buyAmount) / parseFloat(sellAmount)).toFixed(4)} ${buyToken.symbol}` : '---'}
        </span>
      </div>
      ...
      <button 
        ...
        className="w-full bg-gradient-to-r from-haven-pink to-haven-purple py-6 rounded-2xl font-black uppercase text-xs tracking-[0.4em] shadow-[0_10px_40px_rgba(255,0,122,0.3)] disabled:grayscale disabled:opacity-50 transition-all"
      >
