...
const PAIR_ABI = [
  ...
  { "inputs": [ { "name": "owner", "type": "address" }, { "name": "spender", "type": "address" } ], "name": "allowance", "outputs": [ { "name": "allowance", "type": "uint256" } ], "stateMutability": "view", "type": "function" }
] as const;
...
export default function Liquidity() {
  ...
  const { data: havBalance, refetch: refHAV } = useBalance({ address, query: { enabled: mounted && !!address } });
  const { data: tokenBalance, refetch: refToken } = useBalance({ address, token: tokenOut.address as `0x${string}`, query: { enabled: mounted && !!address } });
  ...
  const handleAction = () => {
    ...
    if (!allowanceToken || (allowanceToken as bigint) < valToken) {
      writeContract({
        address: tokenOut.address as `0x${string}`, abi: PAIR_ABI, functionName: 'approve',
        args: [ROUTER_ADDRESS as `0x${string}`, parseUnits('999999999999', tokenOut.decimals)],
      });
      return;
    }

    writeContract({
      address: ROUTER_ADDRESS as `0x${string}`, abi: ROUTER_ABI, functionName: 'addLiquidityETH',
  ...
  const handleRemove = () => {
    if (!lpBalance || (lpBalance as bigint) === BigInt(0) || !pairAddress) return;
    const amountToRemove = ((lpBalance as bigint) * BigInt(removePercent)) / BigInt(100);

    writeContract({
      address: pairAddress as `0x${string}`, abi: PAIR_ABI, functionName: 'approve',
      args: [ROUTER_ADDRESS as `0x${string}`, amountToRemove],
    }, {
      onSuccess: () => {
        writeContract({
          address: ROUTER_ADDRESS as `0x${string}`, abi: ROUTER_ABI, functionName: 'removeLiquidityETH',
    ...
    <div className="flex flex-col items-center p-4">
      <div className="w-full max-w-[460px] bg-[#0d0d0d] border border-gray-800 rounded-[30px] p-6 shadow-2xl">
        ...
        <button onClick={handleAction} disabled={isConfirming} className="w-full bg-gradient-to-r from-pink-600 via-purple-600 to-blue-600 text-white font-black py-4 rounded-2xl shadow-lg flex justify-center items-center gap-2 uppercase italic text-sm active:scale-95 transition-transform">
          ...
          {!allowanceToken || (allowanceToken as bigint) < parseUnits(amountToken || '0', tokenOut.decimals) ? `Enable ${tokenOut.symbol}` : 'Supply Liquidity'}
        </button>

        <div className="border-t border-gray-800 mt-8 pt-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xs font-black uppercase italic text-gray-500">Your Position</h3>
            <span className="text-pink-500 text-[10px] font-black italic">
              {lpBalance && (lpBalance as bigint) > BigInt(0) ? formatCompact(formatUnits(lpBalance as bigint, 18), 6) : '0.00'} LP
            </span>
          </div>

          {pairAddress && lpBalance && (lpBalance as bigint) > BigInt(0) ? (
